...
const char atapimodel[50];
char *ret;

extern char buftxt[2048];
extern char *bufftxt;

char * dvdromodel[] = { "TS-H943", "TS-H943A", "GDR3120L", "VAD6038", "DG-16D2S", "DG-16D4S", "DG-16D5S", "DL10N", "DLN10N" };

struct xenon_ata_device2 ata2;
struct xenon_ata_device2 atapi2;

static inline void
xenon_ata_regset(struct xenon_ata_device2 *dev, int reg, uint8_t val) {
	*(volatile uint8_t*)(dev->ioaddress + reg) = val;
}

static inline uint8_t
xenon_ata_regget(struct xenon_ata_device2 *dev, int reg) {
	return *(volatile uint8_t*)(dev->ioaddress + reg);
}

static inline void
xenon_ata_regset2(struct xenon_ata_device2 *dev, int reg, int val) {
	*(volatile uint8_t*)(dev->ioaddress2 + reg) = val;
}

static inline int
xenon_ata_regget2(struct xenon_ata_device2 *dev, int reg) {
	return *(volatile uint8_t*)(dev->ioaddress2 + reg);
}

static inline uint32_t
xenon_ata_read_data(struct xenon_ata_device2 *dev, int reg) {
	return *(volatile uint32_t*)(dev->ioaddress + reg);
}

static inline void
xenon_ata_write_data(struct xenon_ata_device2 *dev, int reg, uint32_t val) {
	*(volatile uint32_t*)(dev->ioaddress + reg) = val;
}

static inline void
xenon_ata_write_data2(struct xenon_ata_device2 *dev, int reg, uint32_t val) {
	*(volatile uint32_t*)(dev->ioaddress2 + reg) = val;
}

static inline void
xenon_ata_wait_busy(struct xenon_ata_device2 *dev) {
	while ((xenon_ata_regget(dev, XENON_ATA_REG_STATUS) & 0x80));
}

static inline void
xenon_ata_wait_drq(struct xenon_ata_device2 *dev) {
	while (!(xenon_ata_regget(dev, XENON_ATA_REG_STATUS) & 0x08));
}

static inline void
xenon_ata_wait_ready(struct xenon_ata_device2 *dev) {
...
static int
xenon_ata_pio_read(struct xenon_ata_device2 *dev, char *buf, int size) {
...
static int
xenon_ata_pio_write(struct xenon_ata_device2 *dev, char *buf, int size) {
	uint32_t *buf32 = (uint32_t *) buf;
	unsigned int i;
...
static int
xenon_ata_dma_read(struct xenon_ata_device2 *dev, char *buf, int size) {
	...
	memdcbst(dev->prds, MAX_PRDS * sizeof (struct xenon_ata_dma_prd2));

	/* setup DMA regs */

	xenon_ata_write_data2(dev, XENON_ATA_DMA_TABLE_OFS2, __builtin_bswap32((uint32_t) dev->prds & 0x7fffffff));
	xenon_ata_regset2(dev, XENON_ATA_DMA_CMD2, XENON_ATA_DMA_WR2);
	xenon_ata_regset2(dev, XENON_ATA_DMA_STATUS2, 0);
	...
	xenon_ata_regset2(dev, XENON_ATA_DMA_CMD2, XENON_ATA_DMA_WR2 | XENON_ATA_DMA_START2);

	/* wait for DMA end */

	while (xenon_ata_regget2(dev, XENON_ATA_DMA_STATUS2) & XENON_ATA_DMA_ACTIVE2);

	/* stop DMA ctrlr */

	xenon_ata_regset2(dev, XENON_ATA_DMA_CMD2, 0);

	if (xenon_ata_regget2(dev, XENON_ATA_DMA_STATUS2) & XENON_ATA_DMA_ERR2) {
...
static void
xenon_ata_dumpinfo(struct xenon_ata_device2 *dev, char *info) {
	char text[41];
	char data[0x80];
	int i;
...
static void
xenon_ata_setlba(struct xenon_ata_device2 *dev, int sector, int size) {
...
static int
xenon_ata_setaddress(struct xenon_ata_device2 *dev, int sector, int size) {
...
xenon_ata_read_sectors(sec_t start_sector, sec_t sector_size, void *buf) {
	...
	struct xenon_ata_device2 *dev = &ata2;
...
#ifndef USE_DMA
	unsigned int sect = 0;
	xenon_ata_regset(dev, XENON_ATA_REG_CMD, XENON_ATA_CMD_READ_SECTORS2_EXT);
	...
	return sect;
#else
	xenon_ata_regset(dev, XENON_ATA_REG_CMD, XENON_ATA_CMD_READ_DMA_EXT2);
	xenon_ata_wait_ready(dev);
...
xenon_ata_write_sectors(lba_t start_sector, int sector_size, const void *buf) {
	...
	/* Read sectors.  */
	xenon_ata_regset(dev, XENON_ATA_REG_CMD, XENON_ATA_CMD_WRITE_SECTORS_EXT2);
	xenon_ata_wait_ready(dev);
	for (sect = 0; sect < sector_size; sect++) {
...
xenon_atapi_identify(struct xenon_ata_device2 *dev) {
	...
	xenon_ata_regset(dev, XENON_ATA_REG_CMD,
			XENON_ATA_CMD_IDENTIFY_PACKET_DEVICE2);
	xenon_ata_wait_ready(dev);
...
static int
xenon_atapi_packet(struct xenon_ata_device2 *dev, char *packet, int dma) {
	...
	xenon_ata_regset(dev, XENON_ATA_REG_CMD, XENON_ATA_CMD_PACKET2);
	xenon_ata_wait_ready(dev);
...
int
xenon_atapi_request_sense2(struct xenon_ata_device2 *dev) {
...
static int
xenon_atapi_inquiry_model2(struct xenon_ata_device2 *dev) {
	...
	xenon_ata_wait_ready(dev);
	if (xenon_ata_pio_read(dev, buf, sizeof (buf))) {
		bufftxt += sprintf(bufftxt, " * Atapi Model: ATAPI inquiry failed ");
		return -1;
	};

	buf[8 + 24] = '\0';
	
 	sprintf(atapimodel, &buf[8]);
    int i;
    
    for(i=0; i<9; i++){
        ret = strstr(atapimodel, dvdromodel[i]);
        if(ret != NULL){
            switch(i){
                case 0:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Toshiba/Samsung TS-H943");
                    break;
                case 1:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Toshiba/Samsung TS-H943A"); 
                    break;
                case 2:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Hitachi/LG GDR-3120L");
                    break;
                case 3:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Philips/BenQ VAD6038"); 
                    break;
                case 4:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Philips/Lite-On DG-16D2S");
                    break;
                case 5:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Philips/Lite-On DG-16D4S");
                    break;
                case 6:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Philips/Lite-On DG-16D5S");
                    break;
                case 7:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Hitachi/LG DL10N");
                    break;
                case 8:
                    bufftxt += sprintf(bufftxt, " * Atapi Model: Hitachi/LG DLN10N");
                    break;
            }
...
void
xenon_atapi_set_modeb2(void) {
	struct xenon_ata_device2 *dev = &atapi2;
...
xenon_atapi_get_dvd_key_tsh943a(unsigned char *dvdkey) {
	struct xenon_ata_device2 *dev = &atapi;
	...
	xenon_ata_wait_ready(dev);
	if (xenon_ata_pio_read(dev, buf, sizeof (buf))) {
		int sense = xenon_atapi_request_sense2(dev);
...
int
xenon_atapi_set_dvd_key2(unsigned char *dvdkey) {
	struct xenon_ata_device2 *dev = &atapi2;
...
xenon_atapi_read_sectors(sec_t start_sector, sec_t sector_size, void *buf) {
	...
	//struct xenon_ata_device *dev = bdev->ctx;
	struct xenon_ata_device2 *dev = &atapi2;
	struct xenon_atapi_read2 readcmd;
...
static int
xenon_ata_identify(struct xenon_ata_device2 *dev) {
	char info[XENON_DISK_SECTOR_SIZE];
	uint16_t *info16 = (uint16_t *) info;
	xenon_ata_wait_busy(dev);

	xenon_ata_regset(dev, XENON_ATA_REG_DISK, 0xE0);
	xenon_ata_regset(dev, XENON_ATA_REG_CMD, XENON_ATA_CMD_IDENTIFY_DEVICE2);
	xenon_ata_wait_ready(dev);

	int val = xenon_ata_pio_read(dev, info, XENON_DISK_SECTOR_SIZE);
	if (val & 4)
		return xenon_atapi_inquiry_model2(dev);
	else if (val)
		return -1;
	...
	dev->addressing_mode = XENON_ATA_CHS2;
	/* Check if LBA is supported.  */
	if (bswap_16(info16[49]) & (1 << 9)) {
		/* Check if LBA48 is supported.  */
		if (bswap_16(info16[83]) & (1 << 10))
			dev->addressing_mode = XENON_ATA_LBA48_2;
		else
			dev->addressing_mode = XENON_ATA_LBA2;
	}

	/* Determine the amount of sectors.  */
	if (dev->addressing_mode != XENON_ATA_LBA48_2)
...
int
xenon_ata_init12(struct xenon_ata_device2 *dev, uint32_t ioaddress, uint32_t ioaddress2) {

	memset(dev, 0, sizeof (struct xenon_ata_device2));
	dev->ioaddress = ioaddress;
	dev->ioaddress2 = ioaddress2;

	dev->prds = memalign(0x10000, MAX_PRDS * sizeof (struct xenon_ata_dma_prd2));
	...
	if (xenon_ata_regget(dev, XENON_ATA_REG_LBALOW) != 0x5A) {
		bufftxt += sprintf(bufftxt, "(no ata device connected)");
		return -1;
	}
...
DISC_INTERFACE xenon_atapi_ops2 = {
...
xenon_ata_init2() {
	...
	*(volatile uint32_t*)0xea001318 &= __builtin_bswap32(0xFFFFFFF0);
	...
	struct xenon_ata_device2 *dev = &ata2;
	memset(dev, 0, sizeof (struct xenon_ata_device2));
	int err = xenon_ata_init12(dev, 0xea001300, 0xea001320);
	if (!err)
		ata_ready = 1;
	return err;
}
...
xenon_atapi_init2() {
	...
	*(volatile uint32_t*)0xea001218 &= __builtin_bswap32(0xFFFFFFF0);
	...
	struct xenon_ata_device2 *dev = &atapi2;
	memset(dev, 0, sizeof (struct xenon_ata_device2));
	int err = xenon_ata_init12(dev, 0xea001200, 0xea001220);
	if (!err)
		atapi_ready = 1;
	return err;
}
