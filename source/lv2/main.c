...
int main(){
	...
	*(volatile uint32_t*)0xea00105c = 0xc000000;
...
#elif defined MC_THEME
	console_set_colors(0X00300000,0Xffffffff); // White text on blue bg
    ...
    bufftxt += sprintf(bufftxt, "***********************    ModConsoles Edition V1.2    ***********************\n");
    ...
    if (cpu_get_key(key)==0)
	{
        ...
        snprintf(buffkey, 2048, "%016llX%016llX", ld(&key[0x0]),ld(&key[0x8]));
    }
	...
	if (kv_get_dvd_key(key)==0)
	{
        ...
        snprintf(buffkey2, 2048, "%016llX%016llX", ld(&key[0x0]),ld(&key[0x8]));
	}
    ...
    mount_all_devices();
    printf(" * Waiting for USB Devices/Files and Looking for files on local media/TFTP...\n\n");
        for(;;){
            if(boolusb == 0){logusb();}
            fileloop();
