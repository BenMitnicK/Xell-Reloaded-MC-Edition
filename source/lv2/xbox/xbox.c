...
static const kventry2 kvlookup[] =
	{
...
int cpu_get_key2(unsigned char *data)
{
...
int get_virtual_cpukey2(unsigned char *data)
{
...
int kv_get_key2(unsigned char keyid, unsigned char *keybuf, int *keybuflen, unsigned char *keyvault)
{
	if (keyid > 0x38)
		return 1;
...
int kv_read2(unsigned char *data, int virtualcpukey)
{
	...
	unsigned char cpu_key[0x10];
        if (virtualcpukey)
            get_virtual_cpukey2(cpu_key);
        else
            cpu_get_key2(cpu_key);
	//print_key2("kv_read: cpu key", cpu_key);

	unsigned char hmac_key[0x10];
	memcpy(hmac_key, data, 0x10);
	//print_key2("kv_read: hmac key", hmac_key);

	unsigned char rc4_key[0x10];
	memset(rc4_key, 0, 0x10);

	HMAC_SHA1(cpu_key, hmac_key, rc4_key, 0x10);
	//print_key2("kv_read: rc4 key", rc4_key);
...
int kv_get_dvd_key2(unsigned char *dvd_key)
{
	if (KV_FLASH_SIZE == 0)
		return -1; //It's bad data!
	unsigned char buffer[KV_FLASH_SIZE], tmp[0x10], serial[0x10];
	int result = 0;
	//int keylen = 0x10;
    int seriallen = 0x0C;

	result = kv_read2(buffer, 0);
        if (result == 2 && get_virtual_cpukey2(tmp) == 0){
            printf("! Attempting to decrypt DVDKey with Virtual CPU Key !\n");
            result = kv_read2(buffer, 1);
        }
	if (result != 0){
	...
	/*result = kv_get_key2(XEKEY_DVD_KEY, dvd_key, &keylen, buffer);
	if (result != 0){
		printf(" ! kv_get_dvd_key Failure: kv_get_key %d\n", result);
		return result;
	}*/
	result = kv_get_key2(XEKEY_CONSOLE_SERIAL_NUMBER, serial, &seriallen, buffer);
    if (result == 0) print_key_plain (" * Serial Number: ", serial, &seriallen);
    else
    {
	...
	//print_key2("dvd key", dvd_key);
	return 0;

}

/*void print_cpu_dvd_keys2(void)
{
	...
	if (cpu_get_key2(key)==0)
		print_key2(" * your cpu key", key);
	if (xenon_logical_nand_data_ok2() == 0)
	{
		memset(key, '\0',sizeof(key));
		if (get_virtual_cpukey2(key)==0)
			print_key2(" * your virtual cpu key", key);

		memset(key, '\0', sizeof(key));
		if (kv_get_dvd_key2(key)==0)
			print_key2(" * your dvd key", key);
	}
	else
...
unsigned int xenon_get_PCIBridgeRevisionID2()
{
	return ((read32(0xd0000008) << 24) >> 24);
}

unsigned int xenon_get_CPU_PVR2()
{
	unsigned int PVR;
...
unsigned int xenon_get_XenosID2()
{
...
int xenon_get_console_type()
{
    ...
    PCIBridgeRevisionID = xenon_get_PCIBridgeRevisionID2();
    consoleVersion = xenon_get_XenosID2();
    DVEversion = xenon_get_DVE2();
    PVR = xenon_get_CPU_PVR2();
...
/*int xenon_logical_nand_data_ok2()
{
	uint16_t tmp;
...
int xenon_get_logical_nand_data2(void* buf, unsigned int offset, unsigned int len)
{
	if (xenon_logical_nand_data_ok2() == 0)
...
/*unsigned int xenon_get_kv_size2()
{
	unsigned int ret;
...
unsigned int xenon_get_kv_offset2()
{
	unsigned int ret;
